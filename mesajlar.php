...
<!----------------------------------------------------------------------------------------------------------------->	
			...
			function sil(x){
				var durum=confirm("Seçilen mesaj kaydı silinecektir. Emin misiniz?");
				if(durum==1){
					window.location.href="kontrol.php?veri=14&id="+x;
				}
			}
			</script>
<?php 
	...
	$listele=mysqli_query($baglan, "select * from mesaj_tbl") or die("Listeleme Baþarýsýz");
?>
	<center>
	<table border="1" bordercolor="#BB1C00" WIDTH="95%">
	<tr><td colspan=6 align=center bgcolor="#BB1C00"><font color="white"><b>GELEN MESAJLAR</b></font></td></tr>
	<tr style="background-color:#BB1C00;color:white;"><td>SIRA</td><td>KİMDEN</td><td>TELEFON</td><td>GİRİŞ TARİHİ</td><td>MESAJ</td><td>SİL</td></tr>

<?php
	$sayac=1;
	while($oku=mysqli_fetch_assoc($listele))
	{
		echo "<tr><td>$sayac</td><td>".$oku["adsoy"]."</td><td>".$oku["tel"]."</td><td>";
		$bugun=$oku["tarih"];
		$objDateTime = new DateTime($bugun);
		$tarih=$objDateTime->format('d.m.Y');
		echo "$tarih</td><td>".$oku["msj"]."</td><td><a href='javascript:sil(".$oku["id"].")'><img src='images/sil.png' width='20' height='20'></a></td></tr>";		
		$sayac++;
	}
