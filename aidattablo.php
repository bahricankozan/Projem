...
<?php 
	...
	$listele=mysqli_query($baglan, "select * from tablo_tbl") or die("Listeleme Baþarýsýz");
?>
	<center>
	<form name="frm" method="post" action="kontrol.php?veri=7">
	<table border="1" bordercolor="#BB1C00">
	<tr><td colspan="4" align="center">AİDAT TUTARI TABLOSU</td></tr><tr>
<?php
	$sayac=1;
	$i=0;
	$liste=1;
	while($oku=mysqli_fetch_assoc($listele))
	{
		if($sayac<=2)
		{
			echo "<td>".$ay[$i]."</td><td><input type=text name='ay".$liste."' value=".$oku["tutar"]."></td>";
		}
		else
		{
			echo "</tr><tr>";
	...
	echo "<p><center><input type='submit' name='bt1' value='GÜNCELLE' onclick='submit()' style='font-family: sans-serif; font-size: 12pt; background-color:FFF1BF; color: brown'>";
