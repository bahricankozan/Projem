    ...
    <tbody>
      ...
      <tr>
        ...
        <td width="762" valign="top">
			...
			function sil(x){
				var durum=confirm("Seçilen sakin kaydı silinecektir. Emin misiniz?");
				if(durum==1){
					window.location.href="kontrol.php?veri=4&sil="+x;
				}
			}
			</script>			
			
			<table width="650" border="1" bordercolor="#BB1C00" cellspacing="0" cellpadding="0">
          <tbody>
            <tr bgcolor="#BB1C00">
              <td>SIRA</td>
              <td>BLOK-DAİRE</td>
			  ...
			  <?php 
			 	...
			 	$s=1;
			  	while($oku=mysqli_fetch_assoc($listele)){
					echo "<tr><td>$s</td><td>".$oku["blok"]."/".$oku["daire"]."</td><td>".$oku["ad"]."</td><td>".$oku["tel"]."</td><td>".date("d.m.Y", strtotime($oku["tarih"]))."</td><td><a href='javascript:sil(".$oku["id"].")'><img src='images/sil.png' width='15' height='15'></a></td></tr>";
					$s++;
				}
			  	mysqli_close($baglan);
			  ?>
